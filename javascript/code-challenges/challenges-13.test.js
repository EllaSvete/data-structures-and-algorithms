...
// Solution code here...
  let newArr = [...arr];
  let otherArr = newArr.sort((a,b) => b.length - a.length);
  return arr.indexOf(otherArr[0]);
};
...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map((letter => letter.charAt(0)));
};
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter(newString => newString.includes(`:)`));
};
